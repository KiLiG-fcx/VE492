...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        qvalues=list()
        if len(legal_action)==0: # no legal action, qvalue=0
            return 0.0
        else:
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legal_action)==0:
            return None
        for action in legal_action:
            act_ind=legal_action.index(action)
            # compute each qvalue for action
            qvalues[act_ind]=self.getQValue(state,action)
        return legal_action[qvalues.argMax()]
        util.raiseNotDefined()
